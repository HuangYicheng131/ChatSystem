...
bool Redis::connect()
{
    ...
    cout << "connect rdis-server success!" << endl;
    return true;
}
...
{
    // subscribe本身会造成线程阻塞，等待通道发送消息，这里只做订阅通道，不接受通道消息
    // 通道消息接收专门在oberve_channel_message函数中的独立线程进行
...
bool Redis::unsubscribe(int channel)
{
    ...
    while(!done){
        if(REDIS_ERR == redisBufferWrite(this->_subscribe_context, &done)){
            cerr << "unnsubscribe command failed!" << endl;
            return false;
        }
    }
